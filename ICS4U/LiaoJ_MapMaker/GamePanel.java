...
public class GamePanel extends JPanel implements ActionListener, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener {
	...
	public GamePanel(MapMaker mapMaker) {
		...
		BufferedImage fillPattern = new BufferedImage(20, 20, BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = (Graphics2D) fillPattern.getGraphics();
		for(int i = 0; i < 40; i += 10) {
			g.drawLine(0, i, i, 0);
			g.drawLine(0, i - 20, 40 - i, 20);
		}

		paint = new TexturePaint(fillPattern, new Rectangle2D.Double(0, 0, 20, 20));
